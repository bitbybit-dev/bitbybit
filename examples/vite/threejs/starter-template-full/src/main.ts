...
async function start() {
    // --- 1. Initialize Three.js Scene with Orbit Camera using the library helper ---
    // This creates scene, renderer, lights, shadows, ground, and orbit camera in one call
    ...
    sceneOptions.orbitCameraOptions.inertiaFactor = 0.1;
    sceneOptions.orbitCameraOptions.distanceSensitivity = 0.15;

    const { scene, startAnimationLoop } = initThreeJS(sceneOptions);

    // Start the animation loop (handles camera updates and rendering)
    startAnimationLoop();

    // Create an instance of BitByBitBase
