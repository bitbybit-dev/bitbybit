...
// Helper to apply depth bias to materials in an entity hierarchy
...
export const createShapeLod2 = async (
    ...
    if (scene && bitbybit) {
        ...
        options.faceColour = model.color1;

        const groups1Promises = compounds1ToDraw.map((comp) =>
            bitbybit.draw.drawAnyAsync({ entity: comp, options })
        );

        const options2 = new Inputs.Draw.DrawOcctShapeOptions();
        options2.precision = model.finalPrecision;
        options2.drawEdges = model.drawEdges;
        options2.drawFaces = model.drawFaces;
        options2.drawVertices = false;
        options2.edgeWidth = 20;
        options2.edgeColour = "#000000";
        options2.faceColour = model.color2;

        const groups2Promises = compounds2ToDraw.map((comp) =>
            bitbybit.draw.drawAnyAsync({ entity: comp, options: options2 })
        );

        const groups1 = await Promise.all(groups1Promises);
        const groups2 = await Promise.all(groups2Promises);

        // Apply depth bias and disable shadow casting on edge entities
        [...groups1, ...groups2].forEach((g) => {
            // Apply small depth bias to prevent z-fighting
        ...
        groups1.forEach((g) => group1.addChild(g));
        scene.addChild(group1);

        const group2 = new Entity("group2");
        groups2.forEach((g) => group2.addChild(g));
...
export const createShapeLod1 = async (
    ...
    if (scene && bitbybit) {
        ...
        options.drawVertices = false;
        options.edgeWidth = 20;
        options.edgeColour = "#ffffff";
        options.faceColour = model.color1;
        options.faceOpacity = 0.5;
        ...
        const options2 = new Inputs.Draw.DrawOcctShapeOptions();
        options2.precision = 0.01;
        options2.drawEdges = true;
        options2.drawFaces = true;
        options2.drawVertices = false;
        options2.edgeWidth = 20;
        options2.edgeColour = "#ffffff";
        options2.faceColour = model.color2;
        options2.faceOpacity = 0.5;
        ...
        // Disable shadow casting on dimensions
        if (dimensionsGroup.render) {
        ...
        const group1 = new Entity("group1");
        group1.addChild(grp1);
        scene.addChild(group1);
        const group2 = new Entity("group2");
        group2.addChild(grp2);
        scene.addChild(group2);
