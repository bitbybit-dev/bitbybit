...
<body>
    ...
    <div id="overlay" class="active">
        ...
        <button id="validate-btn" style="background:transparent;color:#ff6b6b;border:1px solid rgba(255,107,107,0.4) !important">Test Validation (Invalid Params)</button>
        <div id="status"></div>
    </div>
    ...
    <script type="module">
            ...
            fetch("/api/task/" + encodeURIComponent(existingTaskId))
                ...
                .then(data => {
                    if (data.downloads && data.downloads.length) {
                        const glb = data.downloads.find(d => d.format === "glb");
                        if (glb) loadGlb(glb.downloadUrl);
                        else throw new Error("No GLB in task results");
        ...
        btn.addEventListener("click", async () => {
            ...
            try {
                ...
                if (!res.ok || !data.downloads || !data.downloads.length) {
                    throw new Error(data.error || "Generation failed");
                }

                const glbDownload = data.downloads.find(d => d.format === "glb");
                if (!glbDownload) {
                    throw new Error("No GLB format in results");
                }
                ...
                loadGlb(glbDownload.downloadUrl);
            } catch (e) {
                statusEl.className = "error";
                statusEl.textContent = "Error: " + e.message;
                btn.disabled = false;
        ...
        batchBtn.addEventListener("click", async () => {
            ...
            try {
                ...
                if (!res.ok || !data.subTasks || !data.subTasks.length) {
                    throw new Error(data.error || "Batch generation failed");
                }

                // Extract GLB URLs from each sub-task's downloads
                const glbUrls = data.subTasks
                    .map(st => st.downloads.find(d => d.format === "glb"))
                    .filter(d => d != null)
                    .map(d => d.downloadUrl);

                // Load all GLBs side by side with an offset
                const spacing = 15;
                const startX = -spacing * (glbUrls.length - 1) / 2;
        ...
        // Validation demo handler
        validateBtn.addEventListener("click", async () => {
            statusEl.className = "";
            statusEl.textContent = "Sending intentionally invalid params…";

            try {
                const res = await fetch("/api/validate-demo", { method: "POST" });
                const data = await res.json();

                if (data.caught === "BitbybitValidationError") {
                    statusEl.className = "error";
                    statusEl.textContent = "✅ Validation caught the error before API call: " + data.message;
