...
Four more options are worth knowing:
...
- **Show what each part costs** adds a price column beside the quantities. It is off to begin with, because the list is a build sheet before it is a quote and plenty of shops would rather show one total than itemise it. A part that adds nothing on top of the base price is listed with its price column left empty rather than as zero, which would read as something the shopper is getting free. Nothing shows at all while pricing is switched off.
...
**Breakdown style** decides how that breakdown reads, and it only appears once you have switched the breakdown on.

- *One line per charge* is the original, and stays the default. Every charge - the base price, each priced option, each formula, each part - gets one line stating what it is and what it adds.
- *Draw the parts, with pictures and quantities* gives the charges that come from your parts the same treatment the Parts list gives them: the product's picture if you want it, then the name, then how many and what it adds. Everything else stays a plain line. It is the one to reach for when your product is priced from its bill of materials and your products carry good photography, because then the breakdown is the part of the panel a shopper actually reads.

Choosing the second gives you the same **Show how many of each**, **Show each part's picture** and **Group rows** settings the Parts list has, and they mean exactly the same things there.
