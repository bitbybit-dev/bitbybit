...
| *"the configured pricing rules are ... over what is needed to verify a cart at checkout."* | Your pricing has more priced options and visibility conditions than checkout can carry in its rules. Reduce them. Variant matrix charging carries no rules at all; Bundled parts and Single line change how the cart is built, not how much checkout has to work out. |
| *"even a single configured item cannot be verified at checkout: working out this project's prices costs more instructions than the checkout Function is given ..."* | Checkout walks every priced option, part, formula and included product, and the conditions on them, for each configured item - and this project has more of that than it can finish. Use fewer parts and simpler conditions, prefer a plain number to an expression, and reuse one expression across parts rather than writing a different one each time. With Bundled parts, every part checkout nests also costs it, so Single line charging holds more. |
