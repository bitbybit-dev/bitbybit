---
sidebar_position: 4
title: "Asset Security and Public Access"
...
**3D assets work exactly like images on your website.** Just as shoppers can right-click and save product photos from your store, anyone with technical knowledge can download your 3D models, textures, and other assets.

### The Technical Reality

When a shopper visits your product page with a 3D configurator:
...
- This is necessary for your shoppers' browsers to load the assets
...
:::warning Apply this to the asset path only
A wildcard origin is fine on a bucket or path that holds nothing but public 3D assets. Do not apply it at the root of a server that also serves anything private. Narrowing the origin to your store's domain is better still.
:::
...
- Focus on shopper-facing aesthetics rather than technical precision

✅ **Finished Product Representations**
- Show what shoppers see, not how it's made
...
- Represents what shoppers see, not engineering details

## What Else Is Public: Your Configuration

Your models are not the only thing published. When you publish a project, the **configuration itself** is uploaded to a public address that every product page downloads. It has to be - it is what draws the option panel and works out the price in the shopper's browser.

That file contains what the configurator needs in order to run:

- your base price, every option's amount, and your pricing formulas;
- your logic rules and conditions;
- the products you have linked, including their ids, titles, images and the prices recorded for them;
- your parts, with their labels, quantities and prices;
- manufacturer codes you typed onto options.

One field is deliberately kept out of it: a **part's SKU**, which is stripped before the file is uploaded and reaches only you, on the order and its exports.

Everything else on that list is readable by anyone who views your product page source. None of it is a security hole - a shopper can already see prices on the page - but treat it the way you would treat a published price list. In particular, do not type supplier names, cost codes or internal references into a label or a manufacturer code field expecting them to stay private. See [Parts](/learn/3d-bits/composer/gui/parts#what-is-published-and-what-is-not).
...
- Don't reveal manufacturing secrets
- Focus on shopper experience, not technical specs
...
While Shopify may allow you to upload large files, **just because you can doesn't mean you should**. Large 3D files significantly impact your shoppers' experience:

**Performance Impact:**
- Slower page load times
- Longer wait before shoppers can interact with configurators
...
See [Preparing GLTF Assets](/learn/3d-bits/3d-assets/preparing-gltf) for optimization guidance.
...
## Downloads and Shopper Access

### What Shoppers Can Access
...
**And you can choose to hand it over deliberately.** The **Download the 3D model (GLB)** button action (Pro) gives the shopper a file of their configuration on purpose. It exports what is visible with the chosen materials baked in, so it reveals no more than the models you already published - but treat it as a decision, not a default, and leave it off products whose geometry you would rather not put in someone's hands in one click.
...
**Q: Can we use different models for different shoppers?**
...
A: Because shoppers' browsers need to download the files to display your configurator. "Private web content" is a contradiction-if it's on the web, it's accessible.
...
**Now that you understand asset security and public accessibility**, you can make informed decisions about what 3D content to create and publish. The next step is learning how to [prepare optimized GLTF assets](/learn/3d-bits/3d-assets/preparing-gltf) that are both visually appealing and safe for public distribution.
