...
In the Composer, open the **GUI** tab and find the **Pricing** section. Switch pricing on.
...
:::tip Your product's own price is a floor
With **Product lines** charging, the Shopify product's own price is part of every configured order, so no configuration can cost less than it. If your base price is lower, publishing warns you and suggests the two fixes: set the Shopify product's price to **0** and let the configurator carry the whole amount, or use **Variant matrix** or **Single line** charging instead.
:::
...
Use the Composer's preview to change the option back and forth. The total updates as you go, and the breakdown shows each part that makes it up.
...
| Message | What to do |
|---|---|
| A linked product has no price | Open that option in the Composer and re-link it, or give it a plain amount instead |
