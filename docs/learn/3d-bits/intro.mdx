...
3D Bits provides a 3D canvas that responds to changes on your product page — when customers select options, the 3D model updates in real-time. It works with your theme's own option selectors and with third-party options apps, so your configurator feels native to your store rather than replacing what you already built.

3D Bits also **prices what the shopper configured and charges it through Shopify** at checkout — a base price, amounts per option, formulas over a slider, your own products linked to a choice. For most stores that removes the need for a separate pricing app entirely; see [Pricing](/learn/3d-bits/pricing). And where you would rather keep Shopify variants or an options app you already use, that is fully supported too — switch configurator pricing off for those products and mix the two across your catalogue.
