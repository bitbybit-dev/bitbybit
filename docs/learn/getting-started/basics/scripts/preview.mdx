...
![The project details page showing a "Preview" button (with an eye icon) next to a script, used to open the script in full-screen preview mode.](https://bitbybit.dev/files/site/start/general/projects/scripts/open-script-preview-from-project-details.jpeg "Button to preview a script from a public project's details page")
...
2.  **Using the "Preview" Button in Project Management:**
    ...
    *   Clicking this button will open a new browser tab with the script running in full-screen preview mode. The URL in this new tab's address bar is your shareable preview link.
    ...
    ![The project management panel within the editor, highlighting the "Preview" button for the current script.](https://bitbybit.dev/files/site/start/general/projects/scripts/preview-button.jpeg "Menu button to open the script in preview mode")
