...
<BitByBitRenderCanvas
  ...
  iframeUrl="https://stackblitz.com/edit/bitbybit-threejs-runner-occt-bottle?embed=1&file=index.html&theme=dark"
...
<html lang="en">
  <head>
    <title>Bitbybit Runner ThreeJS - Parametric Bottle</title>
    <meta charset="utf-8" />
    ...
    <script type="module">
      ...
      const { bitbybit, Bit, camera, scene, renderer, THREEJS } =
      ...
      // Higher pixel ratio for sharper rendering
      renderer.setPixelRatio(window.devicePixelRatio * 1.5);

      // Enable tone mapping for PBR rendering
      ...
      // Create lights for subtle PBR look
      function createLights() {
        // Soft ambient light for global illumination
        const ambient = new THREEJS.AmbientLight(0xffffff, 0.4);
        scene.add(ambient);

        // Hemisphere light for natural sky/ground lighting
        const hemi = new THREEJS.HemisphereLight(0xffffff, 0x444444, 0.6);
        hemi.position.set(0, 20, 0);
        scene.add(hemi);

        // Key light - main directional light with soft shadows
        const keyLight = new THREEJS.DirectionalLight(0xffffff, 1.5);
        ...
        const fillLight = new THREEJS.DirectionalLight(0xffffff, 0.5);
        fillLight.position.set(-model.thickness, model.height * 0.5, -model.width);
        scene.add(fillLight);

        return { ambient, hemi, keyLight, fillLight };
      }
      ...
      async function createBottle(width, height, thickness, color) {
        ...
        lights.keyLight.position.set(thickness * 2, height * 1.5, width);
        lights.fillLight.position.set(-thickness, height * 0.5, -width);
      }
...
In ThreeJS, point lights are created directly:
```javascript
const light = new THREEJS.PointLight(0xffffff, 500);
...
In ThreeJS, use `scene.remove()` to clean up meshes:
```javascript
if (currentGroup) {
    scene.remove(currentGroup);
    currentGroup = null;
