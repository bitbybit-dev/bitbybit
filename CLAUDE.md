...
sources - the shared test configuration does, through `resolve.conditions`, so a suite sees a
sibling's edit without a rebuild - and one that does not resolves `dist/`. The map never reaches npm:
`dist-manifest.mjs` drops it, together with `devDependencies` and `scripts`, and a
...
JSDoc on the public API is a functional input to the visual editors, whose components are generated from it,
...
excluded from the build configs: the test runner compiles it itself, so dist ships no mocks.
...
its public signature, is declared by the emscripten glue, and api-extractor cannot follow the
thousands of ambient consts that file declares; its surface is pinned harder elsewhere, by
`check:worker-api` (the whole API layer is generated from the kernel and must match byte for byte)
and by `check:worker-parity` (the dotted paths, the signatures and the docs on both sides). `check:tarballs`
...
declarations - and fails on a fragment nothing names rather than dropping it. Edit a fragment and run
`npm run gen:inputs`; `check:inputs` fails on a stale assembled file, which being generated is not linted.

`npm test` at the root runs every package suite, after `check:inputs`, `check:worker-api` and `npm run check:worker-parity`:
...
- Kernel-heavy suites need a raised heap; the package scripts already set
  ...
  file, which is what `pool: "forks"` in their config is for: the kernel holds global state and two
  suites sharing one process corrupt each other.
