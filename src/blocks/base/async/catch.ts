...
export function createCatchBlock() {
    ...
    const blockSelector = 'base_promises_catch';

    Blocks[blockSelector] = {
        init() {
            this.appendValueInput('Promise')
                    .setCheck(null)
                    .appendField(resources.block_base_promises_catch_input_promise_one)
                    .appendField(new FieldVariable(resources.block_base_promises_catch_var_error), 'Error')
                    .appendField(resources.block_base_promises_catch_input_promise_two);
            this.appendStatementInput('Catch')
                    .setCheck(null)
                    .appendField(resources.block_base_promises_catch_statement_catch.toLowerCase());
            createDummyAsyncLoadingIndicator(this, resources);
            this.setColour('#fff');
            this.setOutput(false);
            this.setPreviousStatement(true, null);
            this.setNextStatement(true, null);
            this.setTooltip(resources.block_base_promises_catch_description);
        }
    };
