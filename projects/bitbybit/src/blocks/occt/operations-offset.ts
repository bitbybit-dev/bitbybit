...
export function createOffsetBlock(): void {
    ...
    const blockSelector = 'occt_operations_offset';
    ...
    Blocks[blockSelector] = {
        init(): void {
            this.appendValueInput('OccShape')
                ...
                .appendField(resources.block_occt_operations_offset_input_shape);
            this.appendValueInput('Distance')
                .setCheck('Number')
                .setAlign(ALIGN_RIGHT)
                .appendField(resources.block_occt_operations_offset_input_distance.toLowerCase());
            this.appendValueInput('Tolerance')
                .setCheck('Number')
                .setAlign(ALIGN_RIGHT)
                .appendField(resources.block_occt_operations_offset_input_tolerance.toLowerCase());
    ...
    JavaScript[blockSelector] = (block: Block) => {
        ...
        HS.validate(block, block.workspace, makeRequiredValidationModelForInputs(resources, inputs, [
            resources.block_occt_operations_offset_input_shape,
            resources.block_occt_operations_offset_input_distance,
            resources.block_occt_operations_offset_input_tolerance,
        ]));
        ...
        const code = createStandardContextIIFE(block, blockSelector, inputs, true,
            `bitbybit.occt.operations.offset(inputs)`, true
        );
...
function makeRuntimeValidationModel(
    ...
    return [{
        entity: keys[0],
        validations: [
            getRequired(resources, resources.block_occt_operations_offset_input_shape),
        ]
    }, {
        entity: keys[1],
        validations: [
            getRequired(resources, resources.block_occt_operations_offset_input_distance),
        ]
    }, {
        entity: keys[2],
        validations: [
            getRequired(resources, resources.block_occt_operations_offset_input_tolerance),
        ]
    }
    ];
}
