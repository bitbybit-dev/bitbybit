...
export function createCompoundMakeCompoundBlock(): void {
    ...
    const blockSelector = 'occ_compound_make_compound';
    ...
    Blocks[blockSelector] = {
        init(): void {
            this.appendValueInput('OccShapes')
                ...
                .appendField(resources.block_occ_compound_make_compound_input_shapes);
    ...
    JavaScript[blockSelector] = (block: Block) => {
        ...
        HS.validate(block, block.workspace, makeRequiredValidationModelForInputs(resources, inputs, [
            resources.block_occ_compound_make_compound_input_shapes,
        ]));
        ...
        const code = createStandardContextIIFE(block, blockSelector, inputs, true,
            `bitbybit.occ.makeCompound(inputs)`, true
        );
...
function makeRuntimeValidationModel(
    ...
    return [{
        entity: keys[0],
        validations: [
            getRequired(resources, resources.block_occ_compound_make_compound_input_shapes),
        ]
    }];
}
