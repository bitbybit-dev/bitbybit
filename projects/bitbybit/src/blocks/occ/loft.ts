...
export function createLoftBlock(): void {
    ...
    Blocks[blockSelector] = {
        init(): void {
            this.appendValueInput('OccShapes')
                ...
                .appendField(resources.block_occ_loft_input_shapes);
            this.appendValueInput('MakeSolid')
                .setCheck('Boolean')
                .setAlign(ALIGN_RIGHT)
                .appendField(resources.block_occ_loft_input_make_solid.toLowerCase());
            this.setOutput(true, 'OccShape');
            this.setColour('#fff');
            this.setTooltip(resources.block_occ_loft_description);
    ...
    JavaScript[blockSelector] = (block: Block) => {
        ...
        BitByBitBlockHandlerService.validate(block, block.workspace, makeRequiredValidationModelForInputs(resources, inputs, [
            resources.block_occ_loft_input_shapes,
            resources.block_occ_loft_input_make_solid,
        ]));
...
function makeRuntimeValidationModel(
    ...
    return [{
        entity: keys[0],
        validations: [
            getRequired(resources, resources.block_occ_loft_input_shapes),
        ]
    }, {
        entity: keys[1],
        validations: [
            getRequired(resources, resources.block_occ_loft_input_make_solid),
        ]
    }];
}
