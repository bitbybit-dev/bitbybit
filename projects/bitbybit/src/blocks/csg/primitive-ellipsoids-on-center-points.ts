...
export function createPrimitiveEllipsoidsOnCenterPointsBlock(): void {
    ...
    Blocks[blockSelector] = {
        init(): void {
            this.appendValueInput('Centers')
                ...
                .appendField(resources.block_jscad_ellipsoids_on_center_points_input_centers);
            this.appendValueInput('Radius')
                .setCheck('Array')
                .setAlign(ALIGN_RIGHT)
                .appendField(resources.block_jscad_ellipsoids_on_center_points_input_radius.toLowerCase());
            this.appendValueInput('Segments')
                .setCheck('Number')
                .setAlign(ALIGN_RIGHT)
                .appendField(resources.block_jscad_ellipsoids_on_center_points_input_segments.toLowerCase());
            this.setOutput(true, 'Array');
            this.setColour('#fff');
            this.setTooltip(resources.block_jscad_ellipsoids_on_center_points_description);
            this.setHelpUrl(environment.docsUrl + solidConstants.solidShapesHelpUrl + '#' + 'ellipsoidsoncenterpoints');
        }
    };
    ...
    JavaScript[blockSelector] = (block: Block) => {
        ...
        const code = createStandardContextIIFE(block, blockSelector, inputs, true,
            `return bitbybit.jscad.shapes.ellipsoidsOnCenterPoints(inputs);`
        );
