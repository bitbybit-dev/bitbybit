...
export function createPrimitiveGeodesicSphereBlock(): void {
    ...
    JavaScript[blockSelector] = (block: Block) => {
        ...
        const code = createStandardContextIIFE(block, blockSelector, inputs, true,
            `
            const sphere = BitByBit.CSG.primitives.geodesicSphere({center: [inputs.center[0], inputs.center[2], inputs.center[1]], radius: inputs.radius, frequency: inputs.frequency});
            return sphere;
`
        );
