...
export function createHullSolidsBlock(): void {
    ...
    Blocks[blockSelector] = {
        init(): void {
            this.appendValueInput('Solids')
                ...
                .appendField(resources.block_csg_hull_solids_input_solids);
            this.setOutput(true, 'CsgMesh');
            this.setColour('#fff');
            this.setTooltip(resources.block_csg_hull_solids_description);
            this.setHelpUrl('');
        }
    };
    ...
    JavaScript[blockSelector] = (block: Block) => {
        ...
        const code = createStandardContextIIFE(block, blockSelector, inputs, true,
            `
            return BitByBit.CSG.hulls.hull(...inputs.solids);
`
        );
