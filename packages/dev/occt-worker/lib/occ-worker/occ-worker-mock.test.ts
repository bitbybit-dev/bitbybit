...
const A_CALL: DataInput = { action: { functionName: "shapes.solid.createSphere", inputs: { size: 1 } }, uid: "call-1" };

describe("OCCTWorkerMock unit tests", () => {
    let mock: OCCTWorkerMock;

    beforeEach(() => {
        vi.clearAllMocks();
        mock = new OCCTWorkerMock();
    });
    ...
    describe("postMessage", () => {
        ...
        it("should return the worker's answer on its own onmessage", () => {
            ...
            vi.mocked(onMessageInput).mockImplementation((_input, post) => post({ uid: "call-1", result: "a-sphere" }));

            // Act
            mock.postMessage(A_CALL);

            // Assert
            expect(received).toEqual([{ uid: "call-1", result: "a-sphere" }]);
        });
    });
