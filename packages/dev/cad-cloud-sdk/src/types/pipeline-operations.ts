...
// Generated from operation-catalog.json v1.2.0-rc.1
// ---------------------------------------------------------------------------
...
export interface OperationParams {
    ...
    /** Split manifold by plane on various offsets. Each cut takes the part below the plane as a finished piece and carries the part above it to the next, larger offset, so a run of n offsets yields n + 1 pieces and accounts for the whole of the solid. */
    "manifold.manifold.booleans.splitByPlaneOnOffsets": {
        /** Manifold that will be split */
        manifold: unknown | PipelineRef;
        /** The normal vector of the plane to be mirrored over */
    ...
    /** Build an assembly document from a structure definition. Returns the document handle directly - document stays in worker memory. This is the recommended approach for creating assemblies: 1. Define parts with shapes, names, and optional colors 2. Define nodes (assemblies and instances) with hierarchy and transforms 3. Call this method to create the document 4. Query the document or export to STEP/glTF 5. Call deleteDocument() to release memory when done If existingDocument is provided and valid, the document will be cleared and updated instead of creating a new one. This is useful for updating an assembly without allocating a new document each time. When updating an existing document (existingDocument provided): - If `structure.removals` is provided, those labels are removed first - If `structure.partUpdates` is provided, those parts are updated (shape, name, color) - New `parts` and `nodes` are added to the document - If neither `removals` nor `partUpdates` is provided, the document is cleared first (backward compatible) - Use `clearDocument: false` in structure to preserve existing content while adding new parts/nodes */
    "occt.assembly.manager.buildAssemblyDocument": {
    ...
    /** Create an imported part definition that copies a label tree from another document (typically a STEP-loaded document) into the new assembly. Preserves sub-assembly hierarchy, names and colors. The result can be referenced by `partId` from any instance node to place the imported assembly multiple times. */
    "occt.assembly.manager.createImportedPart": {
    ...
    /** Export an assembly document to glTF binary (GLB) format with explicit Draco geometry compression settings. */
    "occt.assembly.manager.exportDocumentToGltfWithDraco": {
        /** Assembly document handle from buildAssemblyDocument or loadStepToDoc */
        document: unknown | PipelineRef;
        /** Whether to trigger a file download in the browser */
    ...
    /** Convert a STEP file to glTF format with full control over all reading, meshing and writer options, plus explicit Draco geometry compression settings. Same fast path as `convertStepToGltfAdvanced` but exposes the 8 Draco knobs. */
    "occt.io.convertStepToGltfAdvancedWithDraco": {
        /** STEP data as string (for plain text files), ArrayBuffer, Uint8Array, File, or Blob. Supports compressed .stpz files - gzip-compressed data is automatically decompressed. */
        stepData: unknown | PipelineRef;
        /** Scale factor for the model. Useful for unit conversion (e.g., 0.001 to convert mm to meters). Set to 1.0 for no scaling. */
    ...
    /** Convert a STEP file to glTF format (binary GLB) with explicit Draco geometry compression settings. Same fast path as `convertStepToGltf` but exposes the Draco knobs of the underlying native function. */
    "occt.io.convertStepToGltfWithDraco": {
        /** STEP data as string (for plain text files), ArrayBuffer, Uint8Array, File, or Blob. Supports compressed .stpz files - gzip-compressed data is automatically decompressed. */
        stepData: unknown | PipelineRef;
        /** Extra post-pass refining triangles that bulge beyond the deflection (slower, set false for speed). */
