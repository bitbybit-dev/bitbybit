...
// Generated from operation-catalog.json v1.1.1 (2026-06-26T07:36:28.869Z)
// ---------------------------------------------------------------------------
...
export interface OperationParams {
    ...
    "occt.assembly.manager.buildAssemblyDocument": {
        ...
        /** Optional array of source document handles referenced by `structure.loadedParts` entries via `sourceDocumentIndex`. Typically these are documents previously loaded with loadStepToDoc. Lifetime of source documents is the caller's responsibility — they are not modified or deleted by buildAssemblyDocument. */
    ...
    "occt.assembly.manager.combineStructure": {
        ...
        /** source document via `sourceDocumentIndex` (matching the order of `sourceDocuments` on buildAssemblyDocument) and copies a label (or all free shapes) into this assembly, preserving sub-assembly hierarchy, names and colors. Instance nodes can then reference */
    ...
    /** (typically a STEP-loaded document) into the new assembly. Preserves sub-assembly hierarchy, names and colors. The result can be referenced by `partId` from any ```typescript const chairDoc = occt.assembly.manager.loadStepToDoc({ stepData }); const chair = occt.assembly.manager.createImportedPart({ id: "chair", sourceDocumentIndex: 0, name: "Chair" }); const i1 = occt.assembly.manager.createInstanceNode({ id: "c1", partId: "chair", name: "Chair 1", translation: [0,0,0] }); const i2 = occt.assembly.manager.createInstanceNode({ id: "c2", partId: "chair", name: "Chair 2", translation: [500,0,0] }); const structure = occt.assembly.manager.combineStructure({ parts: [], nodes: [i1, i2], loadedParts: [chair] }); const doc = occt.assembly.manager.buildAssemblyDocument({ structure, sourceDocuments: [chairDoc] }); ``` */
    "occt.assembly.manager.createImportedPart": {
        id: string | PipelineRef;
    ...
    /** Export an assembly document to glTF binary (GLB) format with explicit Draco geometry compression settings. ```typescript const document = await occt.assembly.manager.buildAssemblyDocument({ structure }); const glbData = await occt.assembly.manager.exportDocumentToGltfWithDraco({ document, meshDeflection: 0.1, useDraco: true, dracoCompressionLevel: 7, tryDownload: true }); ``` */
    "occt.assembly.manager.exportDocumentToGltfWithDraco": {
        /** Assembly document handle from buildAssemblyDocument or loadStepToDoc */
        document: unknown | PipelineRef;
        /** Whether to trigger a file download in the browser */
    ...
    /** Meshes an XCAF document's free (top-level) shapes as one combined mesh, resolving per-face colours into the colorGroups map of the output. */
    "occt.docToMesh": {
    ...
    /** Meshes an XCAF document's free (top-level) shapes into separate meshes (one per shape), resolving per-face colours into each output's colorGroups map. */
    "occt.docToMeshes": {
        /** The XCAF document to mesh. Each of its free (top-level) shapes is meshed into a separate mesh (one array entry), with per-face colours resolved from the document into each colorGroups map. */
        document: unknown | PipelineRef;
        /** Precision of the mesh */
        precision?: number | PipelineRef;
        /** Adjust Y (up) coordinate system to Z (up) coordinate system */
    ...
    /** Convert a STEP file to glTF format with full control over all reading, meshing and writer options, plus explicit Draco geometry compression settings. Same fast path as `convertStepToGltfAdvanced` but exposes the 8 Draco knobs. settings and Draco knobs. */
    "occt.io.convertStepToGltfAdvancedWithDraco": {
        /** STEP data as string (for plain text files), ArrayBuffer, Uint8Array, File, or Blob. Supports compressed .stpz files - gzip-compressed data is automatically decompressed. */
        stepData: unknown | PipelineRef;
        /** Scale factor for the model. Useful for unit conversion (e.g., 0.001 to convert mm to meters). Set to 1.0 for no scaling. */
    ...
    /** Convert a STEP file to glTF format (binary GLB) with explicit Draco geometry compression settings. Same fast path as `convertStepToGltf` but exposes the Draco knobs of the underlying native function. Accepts File, Blob, string, ArrayBuffer, or Uint8Array. */
    "occt.io.convertStepToGltfWithDraco": {
        /** STEP data as string (for plain text files), ArrayBuffer, Uint8Array, File, or Blob. Supports compressed .stpz files - gzip-compressed data is automatically decompressed. */
        stepData: unknown | PipelineRef;
        /** Extra post-pass refining triangles that bulge beyond the deflection (slower, set false for speed). */
    ...
    /** Returns debug info about the edge's curve: type, degree, poles/knots, rational/periodic/closed, parameter range, period, length and end points. */
    "occt.shapes.edge.debugInfo": {
        /** Shape on which action should be performed */
        shape: unknown | PipelineRef;
    };
    ...
    /** Returns debug info about the face's surface: type, U/V degree, poles/knots, U/V rational/periodic/closed, UV bounds, area, planarity, orientation and wire/edge counts. */
    "occt.shapes.face.debugInfo": {
        /** Shape on which action should be performed */
        shape: unknown | PipelineRef;
    };
    ...
    /** Returns debug info about the shell: face/edge counts, total surface area and per-face surface debug info (type, U/V degree, poles/knots, bounds, area, ...). */
    "occt.shapes.shell.debugInfo": {
        /** Shape on which action should be performed */
        shape: unknown | PipelineRef;
    };
    ...
    /** Returns debug info about the solid: face/edge counts, surface area, volume and per-face surface debug info (type, U/V degree, poles/knots, bounds, area, ...). */
    "occt.shapes.solid.debugInfo": {
        /** Shape on which action should be performed */
        shape: unknown | PipelineRef;
    };
    ...
    /** Returns debug info about the wire: edge count, closed flag, total length and per-edge curve debug info (type, degree, poles/knots, rational/periodic, range, length). */
    "occt.shapes.wire.debugInfo": {
        /** Shape on which action should be performed */
        shape: unknown | PipelineRef;
    };
    ...
    /** Creates a closed, achiral BSpline wire through the points whose shape is genuinely symmetric for symmetric inputs (square, triangle, ...) - mirror-symmetric, not merely rotationally symmetric - with no irregular start/end point. Use this when a plain interpolation looks skewed at the seam. */
    "occt.shapes.wire.interpolatePointsSymmetric": {
