...
export class OCCTEdge {
     ...
     * ```
     */
    fromPoints(inputs: Inputs.OCCT.PointsDto): TopoDS_Edge[] {
        const edges = [];
     ...
     * ```
     */
    fromBasePolyline(inputs: Inputs.OCCT.PolylineBaseDto): TopoDS_Edge[] {
     ...
     * ```
     */
    fromBaseTriangle(inputs: Inputs.OCCT.TriangleBaseDto): TopoDS_Edge[] {
     ...
     * ```
     */
    fromBaseMesh(inputs: Inputs.OCCT.MeshBaseDto): TopoDS_Edge[] {
    ...
    /**
     * Makes a closed, smooth B-spline edge that passes through the given points and joins back to
     * the first with the same smoothness everywhere, so a loop has no visible seam.
     *
     * Only the points are used; the other interpolation options are ignored. An empty or degenerate
     * point list throws an error.
     * @param inputs - The points the loop passes through
